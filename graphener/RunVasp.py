...
#        os.system('rm -r -f *')
        for atom in self.atoms:
            for element in atom.split(','):
                if element.find('Vc') == -1 and not os.path.exists(element):
                    os.mkdir(element)
                    os.chdir(element)
                    atomPotcar = "/fslhome/josephsl/fsl_groups/hessgroup/vaspfiles/src/potpaw_PBE/" + element + '/POTCAR'
                    if os.path.exists(atomPotcar):
                        subprocess.call(['cp', atomPotcar, '.'])
                    else:
                        sys.exit('Failed to read POTCAR in makeHexDirectories')   
                    subprocess.call(['ln','-s','/fslhome/josephsl/bin/vasp533', 'vasp533'])              
                    jobFile = open('job','w')
                    jobFile.write("#!/bin/bash\n\n")
            ...
            for element in atom.split(','):
                if element.find('Vc') == -1 and not os.path.exists(element):
                    ...
                    if os.path.exists(atomPotcar):
                        ...
                        sys.exit('Failed to read POTCAR in makeSingleDirectories')    
                    subprocess.call(['ln','-s','/fslhome/josephsl/bin/vasp533', 'vasp533'])                
                    jobFile = open('job','w')
                    jobFile.write("#!/bin/bash\n\n")
                    jobFile.write("#SBATCH --time=03:00:00\n")
